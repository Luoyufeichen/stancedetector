...
public:
	inline void forward(Graph *cg, const vector<PNode>& x, LSTM1Params *targetParams, LSTM1Params *tweetParams, int target_num) {
		if (x.size() == 0) {
		...
		if (_left2right) {
			left2right_forward(cg, x, target_num, targetParams, tweetParams);
		}
		else {

			right2left_forward(cg, x, target_num, targetParams, tweetParams);
		}
	}

protected:
	inline void left2right_forward(Graph *cg, const vector<PNode>& x, int target_num, LSTM1Params *targetParams, LSTM1Params *tweetParams) {
		for (int idx = 0; idx < _nSize; idx++) {
			LSTM1Params *param = idx < target_num ? targetParams : tweetParams;
			_inputgates[idx].setParam(&_param->input);
			_forgetgates[idx].setParam(&_param->forget);
			_outputgates[idx].setParam(&_param->output);
			_halfcells[idx].setParam(&_param->cell);
	...
	inline void right2left_forward(Graph *cg, const vector<PNode>& x, int target_num, LSTM1Params *targetParams, LSTM1Params *tweetParams) {
		/*
		...
		int idx = target_num - 1;
		int _nSize_ = _nSize;
		while (_nSize_--) {
			int i = (idx == -1 ? _nSize - 1 : idx);
			LSTM1Params *param = i < target_num ? targetParams : tweetParams;
			_inputgates[i].setParam(&_param->input);
			_forgetgates[i].setParam(&_param->forget);
			_outputgates[i].setParam(&_param->output);
			_halfcells[i].setParam(&_param->cell);
			_inputgates[i].setFunctions(&fsigmoid, &dsigmoid);
			_forgetgates[i].setFunctions(&fsigmoid, &dsigmoid);
			_outputgates[i].setFunctions(&fsigmoid, &dsigmoid);
			_halfcells[i].setFunctions(&ftanh, &dtanh);
			if (idx == target_num - 1) {
				_bucket.forward(cg, 0);
				//_bucket.node_name = "target bucket" + 

				_bucket.node_name = (boost::format("bucket tartget %1%") % idx).str();

				_inputgates[idx].forward(cg, &_bucket, x[idx]);

				_inputgates[idx].node_name = (boost::format("_inputgates tartget %1%") % idx).str();

				_halfcells[idx].forward(cg, &_bucket, x[idx]);

				_halfcells[idx].node_name = (boost::format("_halfcells tartget %1%") % idx).str();

				_inputfilters[idx].forward(cg, &_halfcells[idx], &_inputgates[idx]);

				_inputfilters[idx].node_name = (boost::format("_inputfilters tartget %1%") % idx).str();

				_cells[idx].forward(cg, &_inputfilters[idx], &_bucket);

				_cells[idx].node_name = (boost::format("_cells tartget %1%") % idx).str();

				_halfhiddens[idx].forward(cg, &_cells[idx]);

				_halfhiddens[idx].node_name = (boost::format("_halfhiddens tartget %1%") % idx).str();

				_outputgates[idx].forward(cg, &_bucket, x[idx]);

				_outputgates[idx].node_name = (boost::format("_outputgates tartget %1%") % idx).str();

				_hiddens[idx].forward(cg, &_halfhiddens[idx], &_outputgates[idx]);

				_hiddens[idx].node_name = (boost::format("_hiddens tartget %1%") % idx).str();
			}
			else if (idx == -1) {
				idx = _nSize - 1;
				_inputgates[idx].forward(cg, &_bucket, x[idx]);

				_inputgates[idx].node_name = (boost::format("_inputgates tweet %1%") % idx).str();

				_forgetgates[idx].forward(cg, &_bucket, x[idx]);

				_forgetgates[idx].node_name = (boost::format("_forgetgates tweet %1%") % idx).str();

				_halfcells[idx].forward(cg, &_bucket, x[idx]);

				_halfcells[idx].node_name = (boost::format("_halfcells tweet %1%") % idx).str();

				_inputfilters[idx].forward(cg, &_halfcells[idx], &_inputgates[idx]);

				_inputfilters[idx].node_name = (boost::format("_inputfilters tweet %1%") % idx).str();

				_forgetfilters[idx].forward(cg, &_cells[0], &_forgetgates[idx]);

				_forgetfilters[idx].node_name = (boost::format("_forgetfilters tweet %1%") % idx).str();

				_cells[idx].forward(cg, &_inputfilters[idx], &_forgetfilters[idx]);

				_cells[idx].node_name = (boost::format("_cells tweet %1%") % idx).str();

				_halfhiddens[idx].forward(cg, &_cells[idx]);

				_halfhiddens[idx].node_name = (boost::format("_halfhiddens tweet %1%") % idx).str();

				_outputgates[idx].forward(cg, &_bucket, x[idx]);

				_outputgates[idx].node_name = (boost::format("_outputgates tweet %1%") % idx).str();

				_hiddens[idx].forward(cg, &_halfhiddens[idx], &_outputgates[idx]);

				_hiddens[idx].node_name = (boost::format("_hiddens tweet %1%") % idx).str();
			}
			else {
				_inputgates[idx].forward(cg, &_hiddens[idx + 1], x[idx]);

				_inputgates[idx].node_name = idx >= target_num ? (boost::format("_inputgates tweet %1%") % idx).str() : (boost::format("_inputgates target %1%") % idx).str();

				_forgetgates[idx].forward(cg, &_hiddens[idx + 1], x[idx]);

				_forgetgates[idx].node_name = idx >= target_num ? (boost::format("_forgetgates tweet %1%") % idx).str() : (boost::format("_forgetgates target %1%") % idx).str();

				_halfcells[idx].forward(cg, &_hiddens[idx + 1], x[idx]);

				_halfcells[idx].node_name = idx >= target_num ? (boost::format("_halfcells tweet %1%") % idx).str() : (boost::format("_halfcells target %1%") % idx).str();

				_inputfilters[idx].forward(cg, &_halfcells[idx], &_inputgates[idx]);

				_inputfilters[idx].node_name = idx >= target_num ? (boost::format("_inputfilters tweet %1%") % idx).str() : (boost::format("_inputfilters target %1%") % idx).str();

				_forgetfilters[idx].forward(cg, &_cells[idx + 1], &_forgetgates[idx]);

				_forgetfilters[idx].node_name = idx >= target_num ? (boost::format("_forgetfilters tweet %1%") % idx).str() : (boost::format("_forgetfilters target %1%") % idx).str();

				_cells[idx].forward(cg, &_inputfilters[idx], &_forgetfilters[idx]);

				_cells[idx].node_name = idx >= target_num ? (boost::format("_cells tweet %1%") % idx).str() : (boost::format("_cells target %1%") % idx).str();

				_halfhiddens[idx].forward(cg, &_cells[idx]);

				_halfhiddens[idx].node_name = idx >= target_num ? (boost::format("_halfhiddens tweet %1%") % idx).str() : (boost::format("_halfhiddens target %1%") % idx).str();

				_outputgates[idx].forward(cg, &_hiddens[idx + 1], x[idx]);

				_outputgates[idx].node_name = idx >= target_num ? (boost::format("_outputgates tweet %1%") % idx).str() : (boost::format("_outputgates target %1%") % idx).str();


				_hiddens[idx].forward(cg, &_halfhiddens[idx], &_outputgates[idx]);

				_hiddens[idx].node_name = idx >= target_num ? (boost::format("_hiddens tweet %1%") % idx).str() : (boost::format("_hiddens target %1%") % idx).str();
